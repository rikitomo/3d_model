...
def global_step(vertices_ref, reconstructed_vertices, rotations, rimd_features, neighbors):
    """
    ...
    for i in range(num_vertices):
        ...
        for j in neighbors[i]:
            if i < j:
                # コタンジェント重み c_ij を計算
                c_ij = 1 / len(neighbors[j])

                # 基準形状でのエッジベクトルと再構築形状でのエッジベクトル
...
def reconstruct_model_from_RIMD_optimized(rimd_features, vertices_ref, neighbors, max_iterations=100, tolerance=1e-3):
    """
    RIMD特徴量からモデルを再構築する関数。
    エネルギーの変化量が5回連続で閾値以下になった場合に終了条件を満たす。
    ...
    for iteration in range(max_iterations):
        ...
        # グローバルステップ
        reconstructed_vertices = global_step(vertices_ref, reconstructed_vertices, rotations, rimd_features, neighbors)
