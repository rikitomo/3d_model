...
class ModelPredictor:
    ...
    def _process_file(self, test_path: Path, before_path: Path):
        ...
        # 予測の実行と評価
        predicted_xy = self._predict_coordinates(model, after_nodes).to("cpu")  # [B, N, 2]

        # lossの計算（x,y座標のみ）
        before_xy = before_nodes[:, 1:3].float().contiguous()  # x,y座標のみ
        before_xy = before_xy.unsqueeze(0)  # バッチ次元を追加
        loss = criterion(predicted_xy, before_xy).item()
        
        # 予測結果の保存
        self._save_prediction(predicted_xy, after_nodes, faces, test_path.stem[:-5])
        return loss
    ...
    def _predict_coordinates(self, model: PCN, after_nodes: torch.Tensor) -> torch.Tensor:
        ...
        # 予測の実行
        with torch.no_grad():
            predicted_xy = model(after_coords)  # [B, N, 2]
        
        print(f"予測データの形状: {predicted_xy.shape}")
        
        return predicted_xy

    def _save_prediction(self, predicted_xy: torch.Tensor, after_nodes: torch.Tensor,
                        faces: torch.Tensor, base_name: str):
        """予測結果の保存"""
        predicted_xy = predicted_xy.squeeze(0)  # バッチ次元を削除 [N, 2]
        # x,y座標は予測値を使用し、z座標は0を使用
        zeros = torch.zeros((predicted_xy.size(0), 1), device=predicted_xy.device)  # z座標用の0
...
def parse_args() -> PredictConfig:
    """コマンドライン引数の解析"""
    parser = argparse.ArgumentParser(description='PCNモデルを使用した予測の実行（x,y座標のみ）')
