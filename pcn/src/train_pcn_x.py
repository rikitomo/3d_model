...
class PCN(nn.Module):
    def __init__(self, num_dense, latent_dim=512):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Linear(1024, 1024),
            nn.ReLU(inplace=True),
            nn.Linear(1024, num_dense)  # x座標のみを出力
        )
    ...
    def forward(self, xyz):
        ...
        coarse = self.mlp(feature_global).reshape(B, N, 1)  # [B, N, 1]（x座標のみ）
...
class ModelTrainer:
    ...
    def _train_epoch(self, train_loader: DataLoader, optimizer: torch.optim.Optimizer):
        ...
        for after_coords, before_coords, _ in tqdm(train_loader, desc="Training"):
            ...
            optimizer.zero_grad()
            pred_coords = self.model(after_coords)  # [B, N, 1]
            before_x = before_coords[:, :, 0:1]  # x座標のみを抽出
            loss = self.criterion(pred_coords, before_x)
